...
def log_ace_result(state):
    ...
    yes_questions = [i+1 for i, ans in enumerate(state.get("ace_responses", [])) if ans == "Yes"]
    ace_score = state.get("ace_score") or compute_ace_score(state.get("ace_responses", []))
