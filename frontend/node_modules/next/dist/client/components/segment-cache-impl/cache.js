...
async function fetchSegmentPrefetchesUsingDynamicRequest(task, route, fetchStrategy, dynamicRequestTree, spawnedEntries) {
    ...
    try {
        ...
        fulfilledEntries = writeDynamicRenderResponseIntoCache(Date.now(), task, response, serverData, route, spawnedEntries);
        // Return a promise that resolves when the network connection closes, so
        // the scheduler can track the number of concurrent network connections.
        return {
            value: null,
...
function writeDynamicRenderResponseIntoCache(now, task, response, serverData, route, spawnedEntries) {
    if (serverData.b !== (0, _appbuildid.getAppBuildId)()) {
        // The server build does not match the client. Treat as a 404. During
        // an actual navigation, the router will trigger an MPA navigation.
        // TODO: Consider moving the build ID to a response header so we can check
    ...
    for (const flightData of flightDatas){
        ...
        if (seedData !== null) {
            ...
            const staleTimeHeaderSeconds = response.headers.get(_approuterheaders.NEXT_ROUTER_STALE_TIME_HEADER);
            const staleTimeMs = staleTimeHeaderSeconds !== null ? parseInt(staleTimeHeaderSeconds, 10) * 1000 : _prefetchcacheutils.STATIC_STALETIME_MS;
            writeSeedDataIntoCache(now, task, route, now + staleTimeMs, seedData, segmentKey, spawnedEntries);
        }
    }
...
function writeSeedDataIntoCache(now, task, route, staleAt, seedData, key, entriesOwnedByCurrentTask) {
    ...
    if (seedDataChildren !== null) {
        for(const parallelRouteKey in seedDataChildren){
            ...
            if (childSeedData !== null) {
                ...
                writeSeedDataIntoCache(now, task, route, staleAt, childSeedData, (0, _segmentvalueencoding.encodeChildSegmentKey)(key, parallelRouteKey, (0, _segmentvalueencoding.encodeSegment)(childSegment)), entriesOwnedByCurrentTask);
            }
        }
    }
}
