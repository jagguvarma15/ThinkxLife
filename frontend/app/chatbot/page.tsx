...
export default function ChatbotPage() {
  const [step, setStep] = useState<"disclaimer" | "userInfo" | "questionnaire" | "results" | "chat" | "ageRestriction">(
    "disclaimer",
  )
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        {step === "disclaimer" && <Disclaimer onAccept={handleAcceptDisclaimer} />}

        {step === "userInfo" && (
          <div className="bg-white rounded-xl p-8 shadow-xl">
            <h1 className="text-3xl font-bold text-center mb-8 text-gray-800">Meet Zoe</h1>
            <p className="text-gray-600 text-center mb-8">
              Hi, I'm Zoe, your AI companion. I'm here to provide a safe space for you to share and receive support.
              Let's start with some basic information.
            </p>
            <UserInfoForm onSubmit={handleUserInfoSubmit} onAgeRestriction={handleAgeRestriction} />
          </div>
        )}
        ...
        {step === "results" && userInfo && (
          <AceResults score={aceScore} userName={userInfo.name} onStartChat={handleStartChat} />
        )}

        {step === "chat" && userInfo && <ChatInterface userName={userInfo.name} initialMessage={getInitialMessage()} />}
      </div>
    </div>
  )
}
